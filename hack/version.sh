...
cleanup() {
    if [[ -z ${GIT_VERSION} ]] || [[ -z ${GIT_RELEASE_COMMIT} ]]; then
        echo "Git version: ${GIT_VERSION}. Git release: ${GIT_RELEASE_COMMIT}" >&2
        echo "An error occurred during hack/version.sh; Please confirm if git tag <sem_ver> exists." >&2
        exit 1
    fi
}
...
version::ldflags() {
    ...
    function add_ldflag() {
        ...
        local val=${2}
        ldflags+=(
            "-X 'github.com/capi-samples/cluster-api-provider-meta/version.${key}=${val}'"
        )
    }
